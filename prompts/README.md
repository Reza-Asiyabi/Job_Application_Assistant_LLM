...
| `cv_summary.md` | Generate → CV Summary | `cv_context`, `job_description`, `eval_block`, `limit_note` |
| `cover_letter.md` | Generate → Cover Letter | `cv_context`, `job_description`, `eval_block`, `company_info`, `role_info`, `tone_instruction`, `limit_note` |
| `tone_hybrid.md` / `tone_research.md` / `tone_engineering.md` | Cover letter tone directive (inserted as `tone_instruction`) | — |
| `application_question.md` | Q&A page | `cv_context`, `job_description`, `eval_block`, `question`, `limit_note` |
| `refine.md` | Refine button (follow-up on any output) | `instruction` |
...
- `limit_note` — hard word-limit constraint when the user sets one (empty otherwise)
- `instruction` — the user's refinement request (refine continues the previous
  conversation, so the original output is already in context)
